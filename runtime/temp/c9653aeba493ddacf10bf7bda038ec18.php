<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:57:"D:\file\public/../application/index\view\index\index.html";i:1543886382;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	...
	.create-btn{
		...
		line-height:45px;
		cursor:default;
		background:#3385ff url('/static/img/icon/plus.png') no-repeat 2px center;
	...
	.no-data{
		width:100%;
		height:450px;
		background:url('/static/img/icon/empty.png') no-repeat center center;
	...
	.dep-name .fold{
		background:url('/static/img/icon/folder.png') no-repeat 0 center;
		background-size:20px;
	}
	.dep-name .fold-icon{
		background:url('/static/img/icon/arrow-right.png') no-repeat 0 center;
		background-size:20px;
	}
	.dep-name .fold-active{
		background:url('/static/img/icon/arrow-down.png') no-repeat 0 center;
		background-size:20px;
	}
	.hide{
		display:none;
	...
	.group-item .yes{
		...
		width:20px;
		height:20px;
		background:url('/static/img/icon/yes.png') no-repeat center center;
		background-size:20px 20px;
		margin-top:7px;
	}
	.group-item .no{
		...
		width:20px;
		height:20px;
		background:url('/static/img/icon/no.png') no-repeat center center;
		background-size:20px 20px;
		margin-top:7px;
	}
	.upload-main .line{
	...
	.upload-label{
		...
		white-space: nowrap;
		height:50px;
		background:#fff url('/static/img/icon/upload.png') no-repeat 10px center;
	...
	.user{
		float:right;
		background:url('/static/img/icon/user.png') no-repeat 0 center;
		text-indent:20px;
	...
	.edit-icon{
		background:#3385ff url('/static/img/icon/edit.png') no-repeat 5px center;
		background-size:20px;
	}
	.delete-icon{
		background:#3385ff url('/static/img/icon/delete.png') no-repeat 5px center;
		background-size:20px;
	}
	/*authorized-member*/
	.authorized-member{
...
<body>
	...
	<div class="upload-bg">
		...
		<div class="upload-box u-box">
			<form name="form" id="form" method="post" action="/index/index/upload" enctype="multipart/form-data">
				<div class="upload-header">上传文件</div>
				<div class="upload-main">
					<div id="file-area">
						
						<div class="file-line">
							<label for="file" class="upload-label">点我选择文件上传</label><input type="file" name="files[]" class="file" id="file"/>
						</div>
...
var url=protocol+'//'+host+'/project?id=';
$.ajax({
		...
		if(data.state==0){
			for(var i=0;i<rows.length;i++){
				html+='<li class="list-item">';
					...
					html+='<div class="des-line"><span class="des-title">URL:</span><span class="des-con"><a href="/project/index/skip?pname='+rows[i].projectName+'&url='+encodeURIComponent(url+rows[i].projectId)+'" target="_blank">分享项目</a></span></div>';
					html+='<div class="des-line"><span class="des-title">操作:</span><span class="des-con"><a href="javascript:void(0);" class="upload-file" data-id="'+rows[i].projectId+'">上传文件</a>&nbsp;<a href="/index/index/fileList?id='+rows[i].projectId+'">文件列表</a>&nbsp;<a href=""></a></span></div>';
...
$(function(){
	$.ajax({
		url:'/api/index/getDepartmentUser',
		dataType:'json',
		success:function(data){
